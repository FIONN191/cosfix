 ...
 *
 * ⚠️ **这个判定尚未标定，暂时不可全信。** 拿真实场照测过一次：中心裁切
 * 落在白色紧身衣与黑色靴子的交界，本身就是高对比硬边，被判成了
 * oversharpened。当前实现分不清「真实硬边」和「锐化振铃」——两者的
 * 拉普拉斯响应都很大。要真正分开，得检测过冲的**双极性**（边缘一侧
 * 冲高、紧邻的另一侧冲低），而不只是看响应幅值。
 *
 * 在标定之前，metricsBrief 会给这条结论附上「需看图核对」的提示，
 * 不让模型当确定结论用。
 */
